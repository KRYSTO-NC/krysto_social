...
router.get('/:id', auth, async (req, res) => {
  try {
    ...
    if (!post) {
      return res.status(404).json({ msg: 'Poste non trouvée' })
    }
    res.json(post)
  } catch (err) {
    console.error(err.message)
...
router.put('/like/:id', auth, async (req, res) => {
  try {
    ...
    if (
      ...
      return res.status(400).json({ msg: 'Poste déjà likée' })
    }
...
router.put('/unlike/:id', auth, async (req, res) => {
  try {
    ...
    if (
      ...
      return res.status(400).json({ msg: 'Poste non likée' })
    }
    // Get remove index
...
router.post(
  '/report/:id',
  [auth, [check('reason', 'Une raison est requise').not().isEmpty()]],
  async (req, res) => {
    const errors = validationResult(req)
    if (!errors.isEmpty()) {
      return res.status(400).json({ errors: errors.array() })
...
// @route    DELETE api/posts/report/:id/:report_id
// @desc     Delete report
// @access   Private

router.delete('/report/:id/:report_id', auth, async (req, res) => {
  try {
    const post = await Post.findById(req.params.id)
    // Pull out comment
